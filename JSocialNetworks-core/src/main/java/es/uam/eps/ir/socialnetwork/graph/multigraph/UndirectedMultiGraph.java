...
public interface UndirectedMultiGraph<V> extends MultiGraph<V>,UndirectedGraph<V>
{
    ...
    @Override
    public default Stream<Weights<V, Integer>> getIncidentNodesTypesLists(V node)
    {
        return this.getNeighbourNodesTypesLists(node);
    }

    @Override
    public default Stream<Weights<V, Integer>> getAdjacentNodesTypesLists(V node)
    {
        return this.getNeighbourNodesTypesLists(node);
    }
    
    @Override
    public default Stream<Weights<V, Integer>> getNeighbourhoodTypesLists(V node, EdgeOrientation direction)
    {
        return this.getNeighbourNodesTypesLists(node);
    }
