        ...
        if(graph.isDirected())
        {
            ...
            for(Long u : nodes)
            {
                ...
                for(Long v : adjacents)
                {
                    ...
                    {
                        if(writeWeights) dos.writeDouble(weights.get(i));
                        if(writeTypes) dos.writeInt(types.get(i));
                    }   
                }
            }
    ...
    private boolean writeSimple(Graph<Long> graph, OutputStream file, boolean writeWeights, boolean writeTypes) throws IOException
    {
        ...
        if(graph.isDirected())
        {
            ...
            for(Long u : nodes)
            {
                ...
                {
                    dos.writeLong(v);
                    if(writeWeights) dos.writeDouble(graph.getEdgeWeight(u, v));
                    if(writeTypes) dos.writeInt(graph.getEdgeType(u, v));
                }
            }
        }
        else
