 ...
 * 
 */ 
public class PopUnbiasedTestRandomPartition<V> extends AbstractPartition<V>
{
    /**
     ...
     */
    public PopUnbiasedTestRandomPartition(double epsilon, double sigma)
    {
    ...
    public boolean doPartition(Graph<V> graph) 
    {
        try 
        {
            ...
            Comparator<Tuple2od<V>> comparator = (x,y) -> 
            {
                if(x.v2 < y.v2)
                {
                    return 1;
            ...
            {
                List<Weight<V, Double>> links = graph.getIncidentNodesWeights(candidate)
                ...
                {
                   this.train.addEdge(links.get(i).getIdx(), candidate, links.get(i).getValue()); 
                });
                
                IntStream.range(links.size() - num, links.size()).forEach(i ->
                {
                   this.test.addEdge(links.get(i).getIdx(), candidate, links.get(i).getValue()); 
                });
            }
