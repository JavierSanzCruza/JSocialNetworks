 ...
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.graph.ClusteringCoefficientReranker
 */
public class ClusteringCoefficientRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
     ...
     */
    public ClusteringCoefficientRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            ...
            {
                return new ClusteringCoefficientReranker<>(lambda, cutoff, norm, rank, graph);
            });
        });
