...
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.SNCDEGREEGINI;
 ...
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.comm.gini.degree.sizenormalized.SizeNormalizedCompleteCommunityDegreeGiniReranker
 */
public class SizeNormalizedCompleteCommunityDegreeGiniRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    /**
     ...
     */
    public SizeNormalizedCompleteCommunityDegreeGiniRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> comms)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            grid.getOrientationValues(ORIENTATION).forEach(orient -> 
            {
                grid.getBooleanValues(AUTOLOOPS).forEach(autoloop -> 
                {
                    ...
                    {
                        return new SizeNormalizedCompleteCommunityDegreeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms,autoloop, orient);
                    });               
                });
            });
        });
