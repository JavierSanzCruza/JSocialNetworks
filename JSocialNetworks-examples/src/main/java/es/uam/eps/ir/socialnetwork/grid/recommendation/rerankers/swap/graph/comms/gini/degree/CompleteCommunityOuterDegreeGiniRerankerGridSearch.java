...
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.OUTERCDEGREEGINI;
 ...
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.comm.gini.degree.CompleteCommunityOuterDegreeGiniReranker
 */
public class CompleteCommunityOuterDegreeGiniRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
     ...
     */
    public CompleteCommunityOuterDegreeGiniRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> comms)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            grid.getOrientationValues(ORIENTATION).forEach(orient -> 
            {
                grid.getBooleanValues(AUTOLOOPS).forEach(autoloop -> 
                {
                    ...
                    {
                        return new CompleteCommunityOuterDegreeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms,autoloop, orient);
                    });               
                });
            });
        });
