...
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.ICEDGEGINI;
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.SNICEDGEGINI;
 ...
 * to different pairs of communities). The number of links is normalized by the maximum number of links between nodes in both 
 * communities.
 * 
 * @author Javier Sanz-Cruzado Puig
 * @param <U> Type of the users
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.comm.gini.edge.sizenormalized.SizeNormalizedInterCommunityEdgeGiniReranker
 */
public class SizeNormalizedInterCommunityEdgeGiniRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    /**
     ...
     */
    public SizeNormalizedInterCommunityEdgeGiniRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> comms)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            ...
            {
                return new SizeNormalizedInterCommunityEdgeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms);
            });               
