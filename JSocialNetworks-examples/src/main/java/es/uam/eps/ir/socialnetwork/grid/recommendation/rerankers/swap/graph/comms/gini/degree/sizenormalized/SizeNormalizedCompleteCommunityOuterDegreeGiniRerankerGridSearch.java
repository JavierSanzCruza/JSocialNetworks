 ...
 */
package es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.swap.graph.comms.gini.degree.sizenormalized;

import es.uam.eps.ir.socialnetwork.community.Communities;
import es.uam.eps.ir.socialnetwork.graph.Graph;
import es.uam.eps.ir.socialnetwork.grid.Grid;
import es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerGridSearch;
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.OUTERSNCDEGREEGINI;
 ...
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.comm.gini.degree.sizenormalized.SizeNormalizedCompleteCommunityOuterDegreeGiniReranker
 */
public class SizeNormalizedCompleteCommunityOuterDegreeGiniRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    /**
     ...
     */
    public SizeNormalizedCompleteCommunityOuterDegreeGiniRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> comms)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            grid.getOrientationValues(ORIENTATION).forEach(orient -> 
            {
                grid.getBooleanValues(AUTOLOOPS).forEach(autoloop -> 
                {
                    ...
                    {
                        return new SizeNormalizedCompleteCommunityOuterDegreeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms,autoloop, orient);
                    });               
                });
            });
        });
