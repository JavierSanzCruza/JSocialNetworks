...
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.ICDEGREEGINI;
 ...
 */
public class InterCommunityDegreeGiniRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    /**
     ...
     */
    public InterCommunityDegreeGiniRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> comms)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            ...
            {
                rerankers.put(ICDEGREEGINI + "-" + orient + "-" + lambda, () -> 
                {
                    return new InterCommunityDegreeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms, orient);
                });               
            });
        });
