...
/**
 * Class for generating patterns given a graph.
 * @author Javier Sanz-Cruzado Puig
 */
public class PatternGenerator 
{
    /**
     * Program that generates training patterns for the both the new and disappearing prediction problems.
     * @param args Execution arguments
     * <ul>
     *  <li><b>Graph File:</b> First graph (t_0)</li>
     *  <li><b>Class File:</b> Second graph (t_1)</li>
     *  <li><b>Louvain comm.:</b> File containing Louvain communities (for the first graph)</li>
     *  <li><b>Lead. Vector comm.:</b> File containing Leading Eigenvector communities (for the first graph)</li>
     *  <li><b>Infomap comm.:</b> File containing Infomap communities (for the first graph)</li>
    ...
    public static void main(String[] args) throws IOException
    {
        if(args.length < 6)
        {
            System.err.println("Usage: <graphFile> <classFiles> <louvainComm> <leadVectorComm> <infomapComm> <outputFile> <directed>");
            return;
        }
        ...
        // Read the graph
        TextGraphReader<Long> greader = new TextGraphReader<>(false, directed, true, true, "\t", Parsers.lp);
        Graph<Long> graph = greader.read(input, true, false);
        
        // Read the classes: if an edge appears in the graph, the class is negative.
        Graph<Long> classgraph = Adapters.removeAutoloops(greader.read(input, true, false));
        ...
        if(directed)
        {
            ...
            b = System.currentTimeMillis();
            System.out.println("Indegrees prepared (" + (b-a) + " ms.)");
            Map<Long, Double> outDegrees = outDegree.compute(graph);
            b = System.currentTimeMillis();
            System.out.println("Outdegrees prepared (" + (b-a) + " ms.)");
            Map<Long, Double> closenesses = closeness.compute(graph);
            b = System.currentTimeMillis();
            System.out.println("Closenesses prepared (" + (b-a) + " ms.)");
            Map<Long, Double> betweennesses = betweenness.compute(graph);
            b = System.currentTimeMillis();
            System.out.println("Betweennesses prepared (" + (b-a) + " ms.)");
            Map<Long, Double> clustcoefs = clustcoef.compute(graph);
            b = System.currentTimeMillis();
            System.out.println("Local clustering coefficients prepared (" + (b-a) + " ms.)");
        ...
        else
        {
            ...
            Map<Long, Double> outDegrees = outDegree.compute(graph);
            b = System.currentTimeMillis();
            System.out.println("degrees prepared (" + (b-a) + " ms.)");
            Map<Long, Double> closenesses = closeness.compute(graph);
            b = System.currentTimeMillis();
            System.out.println("Closenesses prepared (" + (b-a) + " ms.)");
            Map<Long, Double> betweennesses = betweenness.compute(graph);
