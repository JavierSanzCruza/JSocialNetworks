 ...
 */
package es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.swap.graph;

import es.uam.eps.ir.socialnetwork.graph.Graph;
import es.uam.eps.ir.socialnetwork.grid.Grid;
import es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerGridSearch;
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.CLUSTCOEFCOMPL;
import es.uam.eps.ir.socialnetwork.recommendation.reranking.global.GlobalReranker;
import es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.graph.ClusteringCoefficientComplementReranker;
 ...
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.graph.ClusteringCoefficientComplementReranker
 */
public class ClusteringCoefficientComplementRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    /**
     ...
     */
    public ClusteringCoefficientComplementRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        {
            rerankers.put(CLUSTCOEFCOMPL + "-" + lambda, () -> 
            {
                return new ClusteringCoefficientComplementReranker<>(lambda, cutoff, norm, rank, graph);
            });
        });
