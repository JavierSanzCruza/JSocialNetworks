...
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.OUTERSNICDEGREEGINI;
 ...
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.comm.gini.degree.InterCommunityDegreeGiniReranker
 */
public class SizeNormalizedInterCommunityOuterDegreeGiniRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    /**
     ...
     */
    public SizeNormalizedInterCommunityOuterDegreeGiniRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> comms)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            grid.getOrientationValues(ORIENTATION).forEach(orient -> 
            {
                ...
                {
                    return new SizeNormalizedInterCommunityOuterDegreeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms, orient);
                });               
            });
        });
