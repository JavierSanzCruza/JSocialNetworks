...
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.AVGWEAKNESS;
...
import es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.edge.HeuristicWeaknessReranker;
...
/**
 * Grid search for a heuristic reranker that optimizes the Average Embeddedness of the graph.
 * @author Javier Sanz-Cruzado Puig
 * @param <U> Type of the users
 */
public class HeuristicAverageWeaknessRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    /**
     ...
     */
    public HeuristicAverageWeaknessRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            ...
            {
                rerankers.put(HEURISTICAVGWEAKNESS + "-" + mode + "-" + lambda, () -> 
                {
                    return new HeuristicWeaknessReranker<>(lambda, cutoff, norm, rank, graph, mode);
                });
            });
        });
