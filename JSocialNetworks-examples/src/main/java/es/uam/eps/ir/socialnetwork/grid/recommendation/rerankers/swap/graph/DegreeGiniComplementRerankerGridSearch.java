...
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.DEGREEGINICOMPL;
 ...
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.graph.DegreeGiniReranker
 */
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            grid.getOrientationValues(ORIENTATION).forEach(orient -> 
            {
                ...
                {
                    return new DegreeGiniReranker<>(lambda, cutoff, norm, rank, graph, orient);
                });               
            });
        });
