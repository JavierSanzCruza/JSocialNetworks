...
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.STRONGTIES;
 ...
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.local.communities.StrongTiesReranker
 */
public class StrongTiesRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            ...
            {
                return new StrongTiesReranker<>(lambda, cutoff, norm, graph, comms, rank);
            });
        });
