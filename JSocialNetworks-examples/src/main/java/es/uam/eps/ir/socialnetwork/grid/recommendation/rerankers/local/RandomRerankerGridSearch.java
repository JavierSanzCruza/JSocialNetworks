 ...
 */
package es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.local;

import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.RANDOM;
...
import es.uam.eps.ir.socialnetwork.recommendation.reranking.global.local.LocalRandomReranker;
...
public class RandomRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        {
            rerankers.put(RANDOM + "-" + lambda, () -> 
            {
               return new LocalRandomReranker<>(cutoff, lambda, norm, rank); 
            });
        });
