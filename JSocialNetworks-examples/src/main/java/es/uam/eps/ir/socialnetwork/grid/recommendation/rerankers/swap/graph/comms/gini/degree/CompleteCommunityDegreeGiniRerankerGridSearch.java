...
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.CDEGREEGINI;
 ...
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.comm.gini.degree.CompleteCommunityDegreeGiniReranker
 */
public class CompleteCommunityDegreeGiniRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
     ...
     */
    public CompleteCommunityDegreeGiniRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> comms)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            grid.getOrientationValues(ORIENTATION).forEach(orient -> 
            {
                grid.getBooleanValues(AUTOLOOPS).forEach(autoloop -> 
                {
                    ...
                    {
                        return new CompleteCommunityDegreeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms,autoloop, orient);
                    });               
                });
            });
        });
