...
import static es.uam.eps.ir.socialnetwork.grid.metrics.comm.global.GlobalCommunityMetricIdentifiers.SIZENORMCOMPLETECOMMUNITYEDGEGINI;
import es.uam.eps.ir.socialnetwork.metrics.CommunityMetric;
import es.uam.eps.ir.socialnetwork.metrics.communities.graph.gini.edges.sizenormalized.SizeNormalizedCompleteCommunityEdgeGini;
...
public class SizeNormalizedCompleteCommunityEdgeGiniGridSearch<U> implements GlobalCommunityMetricGridSearch<U> 
{
    ...
    public Map<String, Supplier<CommunityMetric<U>>> grid(Grid grid) 
    {
        ...
        {
            metrics.put(SIZENORMCOMPLETECOMMUNITYEDGEGINI + "_"  + (autoloops ? "autoloops" : "noautoloops"), () -> 
            {
                return new SizeNormalizedCompleteCommunityEdgeGini(autoloops);
            });
        });
