...
import static es.uam.eps.ir.socialnetwork.grid.metrics.comm.global.GlobalCommunityMetricIdentifiers.COMPLETECOMMUNITYEDGEGINI;
import es.uam.eps.ir.socialnetwork.metrics.CommunityMetric;
import es.uam.eps.ir.socialnetwork.metrics.communities.graph.gini.edges.CompleteCommunityEdgeGini;
...
public class CompleteCommunityEdgeGiniGridSearch<U> implements GlobalCommunityMetricGridSearch<U> 
{
    ...
    public Map<String, Supplier<CommunityMetric<U>>> grid(Grid grid) 
    {
        ...
        {
            metrics.put(COMPLETECOMMUNITYEDGEGINI + "_" +  (autoloops ? "autoloops" : "noautoloops"), () -> 
            {
                return new CompleteCommunityEdgeGini(autoloops);
            });
        });
