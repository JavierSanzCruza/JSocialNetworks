...
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.CEDGEGINI;
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.SNCEDGEGINI;
...
/**
 * Grid search for a reranker that reduces the Gini index of the number of links between pairs of communities. Number of links is normalized
 * by the maximum possible number of edges between the different endpoints. The number of links is normalized by the maximum number of links between nodes in both 
 * communities.
 * 
 * @author Javier Sanz-Cruzado Puig
 * @param <U> Type of the users
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.comm.gini.edge.SizeNormalizedCompleteCommunityOuterEdgeGiniReranker
 */
public class SizeNormalizedCompleteCommunityEdgeGiniRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    /**
     ...
     */
    public SizeNormalizedCompleteCommunityEdgeGiniRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> comms)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            grid.getBooleanValues(AUTOLOOPS).forEach(autoloop ->
            {
                ...
                {
                    return new SizeNormalizedCompleteCommunityEdgeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms, autoloop);
                });               
            });
        });
