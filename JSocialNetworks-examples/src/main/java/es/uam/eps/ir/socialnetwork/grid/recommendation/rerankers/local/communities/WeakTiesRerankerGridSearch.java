 ...
 */
package es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.local.communities;
 ...
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.local.communities.WeakTiesReranker
 */
public class WeakTiesRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    /**
     ...
     */
    public WeakTiesRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> comms)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        {
            rerankers.put(WEAKTIES + "-" + lambda, () -> 
            {
                return new WeakTiesReranker<>(lambda, cutoff, norm, rank, graph, comms);
            });
        });
