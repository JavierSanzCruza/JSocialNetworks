...
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.OUTERSCEDGEGINI;
 ...
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.comm.gini.edge.SemiCompleteCommunityEdgeGiniReranker
 */
public class SemiCompleteCommunityOuterEdgeGiniRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    /**
     ...
     */
    public SemiCompleteCommunityOuterEdgeGiniRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> comms)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            ...
            {
                rerankers.put(OUTERSCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () -> 
                {
                    return new SemiCompleteCommunityOuterEdgeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms, autoloop);
                });               
            });
        });
