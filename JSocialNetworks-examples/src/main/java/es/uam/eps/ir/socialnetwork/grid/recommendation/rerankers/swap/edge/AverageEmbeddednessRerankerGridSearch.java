 ...
 */
public class AverageEmbeddednessRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    /**
     ...
     */
    public AverageEmbeddednessRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        {
            rerankers.put(AVGEMBEDDEDNESS + "-" + lambda, () -> 
            {
                return new EmbedednessReranker<>(lambda, cutoff, norm, rank, graph);
            });
        });
