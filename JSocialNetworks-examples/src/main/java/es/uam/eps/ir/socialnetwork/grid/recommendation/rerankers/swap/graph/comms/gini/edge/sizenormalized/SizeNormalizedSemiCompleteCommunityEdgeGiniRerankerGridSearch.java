...
import static es.uam.eps.ir.socialnetwork.grid.recommendation.rerankers.RerankerIdentifiers.SNSCEDGEGINI;
 ...
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.comm.gini.edge.SizeNormalizedSemiCompleteCommunityOuterEdgeGiniReranker
 */
public class SizeNormalizedSemiCompleteCommunityEdgeGiniRerankerGridSearch<U> implements RerankerGridSearch<U> 
{
    /**
     ...
     */
    public SizeNormalizedSemiCompleteCommunityEdgeGiniRerankerGridSearch(int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> comms)
    {
        this.cutoff = cutoff;
        this.norm = norm;
        this.graph = graph;
    ...
    public Map<String, Supplier<GlobalReranker<U, U>>> grid(Grid grid) 
    {
        ...
        grid.getDoubleValues(LAMBDA).forEach(lambda -> 
        {
            ...
            {
                rerankers.put(SNSCEDGEGINI + "-" + (autoloop ? "autoloops" : "noautoloops") + "-" + lambda, () -> 
                {
                    return new SizeNormalizedSemiCompleteCommunityEdgeGiniReranker<>(lambda, cutoff, norm, rank, graph, comms, autoloop);
                });               
            });
        });
