...
import es.uam.eps.ir.socialnetwork.utils.indexes.MonteCarloGiniCollection;
 ...
 */
public class MonteCarloUserFeatureGini<U extends Serializable,I extends Serializable,P> extends AbstractFeatureGlobalSimulationMetric<U,I,P>
{
    /**
    * Metric name
    */
    private final static String GINI = "mc-user-feature-gini";
     ...
     */
    public MonteCarloUserFeatureGini(String parameter,boolean userparam, boolean unique, int numMC) 
    {
    ...
    protected void updateUserParam(Iteration<U, I, P> iteration) 
    {
        ...
        this.mcList.update(new Double(this.freqsum - auxSum).intValue());
    }
    ...
    protected void initialize() 
    {
        if(this.initialized == false && this.data.doesFeatureExist(this.getParameter()) && this.data.isUserFeature(this.getParameter()) == this.usesUserParam())
        {
            ...
            this.values.add(0.0);
            this.mcList = new MonteCarloGiniCollection(new Long(this.size).intValue(), 0, this.numMC);
    ...
    public double calculate() 
    {
        ...
        {
            double gini = this.mainsum/(this.freqsum * (this.size - 1.0));
            gini = (gini - this.mcList.averageValue())/(this.mcList.maxValue() - this.mcList.averageValue());
            return (1.0 - gini)/2.0;
        }
    }
