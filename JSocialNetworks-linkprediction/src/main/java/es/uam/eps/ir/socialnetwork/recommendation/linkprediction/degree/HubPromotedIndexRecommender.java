...
/**
 * Hub Promoted Index (HPI) method for link prediction adaptation.
 * 
 ...
 */
public class HubPromotedIndexRecommender<U> extends UserFastRankingRecommender<U>
{
     ...
     */
    public HubPromotedIndexRecommender(FastGraph<U> graph, EdgeOrientation uSel, EdgeOrientation vSel) 
    {
        super(graph);
        this.uSel = uSel;
        this.vSel = vSel;
    ...
    public Int2DoubleMap getScoresMap(int uidx) 
    {
        ...
        this.uIndex.getAllUsers().forEach(v -> {
            ...
            long denominator = Math.min(uNeigh.stream().count(), this.getGraph().getNeighbourhood(v, vSel).count());
