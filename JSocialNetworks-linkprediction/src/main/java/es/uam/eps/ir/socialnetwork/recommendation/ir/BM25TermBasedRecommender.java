...
import es.uam.eps.ir.socialnetwork.recommendation.UserFastRankingUpdateableRecommender;
 ...
 */
public class BM25TermBasedRecommender<U> extends UserFastRankingUpdateableRecommender<U> 
{
    /**
     ...
     */
    public BM25TermBasedRecommender(FastGraph<U> graph, EdgeOrientation uSel, EdgeOrientation vSel, EdgeOrientation dlSel, double b, double k) 
    {
        super(graph);
    ...
    public void updateAddUser(U u)
    {
        if(!this.graph.containsVertex(u))
        {
            ...
            this.rsj.keySet().stream().forEach(widx -> 
            {
               this.rsj.addTo((int) widx, Math.log(1.0 + 1.0/(this.numUsers - this.wLengths.get((int) widx) + 0.5)));
            });
