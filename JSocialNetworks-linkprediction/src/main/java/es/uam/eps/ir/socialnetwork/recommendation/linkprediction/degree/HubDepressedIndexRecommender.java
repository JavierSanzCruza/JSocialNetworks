...
/**
 * Hub Depressed Index (HDI) method for link prediction adaptation.
 * 
 * 
 * @see es.uam.eps.ir.socialnetwork.recommendation.linkprediction.degree.HubPromotedIndexRecommender
 * 
 * @author Javier Sanz-Cruzado Puig
 * 
 * @param <U> type of the users
 */
public class HubDepressedIndexRecommender<U> extends UserFastRankingRecommender<U>
{
     ...
     */
    public HubDepressedIndexRecommender(FastGraph<U> graph, EdgeOrientation uSel, EdgeOrientation vSel) 
    {
        super(graph);
        this.uSel = uSel;
        this.vSel = vSel;
    ...
    public Int2DoubleMap getScoresMap(int uidx) 
    {
        ...
        this.uIndex.getAllUsers().forEach(v -> {
            ...
            long denominator = Math.max(uNeigh.stream().count(), this.getGraph().getNeighbourhood(v, vSel).count());
