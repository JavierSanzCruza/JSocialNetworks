 ...
 */
package es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap.comm.gini.degree;
...
/**
 * Reranks a recommendation by improving the degree Gini index of the different communities in
 * a community graph.
 * @author Javier Sanz-Cruzado Puig
 * @param <U> Type of the users.
 */
public abstract class CommunityDegreeGiniReranker<U> extends CommunityReranker<U> 
{
    /**
     ...
     */
    public CommunityDegreeGiniReranker(double lambda, int cutoff, boolean norm, boolean rank, Graph<U> graph, Communities<U> communities, boolean autoloops, boolean complete, boolean outer, EdgeOrientation orientation) 
    {
    ...
    private void innerUpdateAddDelete(U user, Tuple2od<U> updated, Tuple2od<U> old)
    {
        ...
        GiniIndex gini = new GiniIndex();
        this.globalvalue = 1.0 - gini.compute(degrees.entrySet().stream().map(entry -> entry.getValue() + 0.0), true, communities.getNumCommunities(), this.sum);
    }
