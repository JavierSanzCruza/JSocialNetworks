...
public class CommOuterPairGiniModifiedReranker<U> extends CommunityReranker<U> {
    ...
    protected double novAddDelete(U u, Tuple2od<U> itemValue, Tuple2od<U> compared) 
    {
        ...
        if(communityGraph.isDirected())
        {
            ...
            {
                if(index == newIdx && !userComm.equals(recommComm) && !userComm.equals(recommComm))
                {
                    atom.addAndGet(1.0);
            ...
            double giniValue = 1.0 - 1.0/gini.compute(stream.boxed(), true, communities.getNumCommunities()*(communities.getNumCommunities()-1), graph.getEdgeCount());
            return giniValue;
        }
        else
        {
