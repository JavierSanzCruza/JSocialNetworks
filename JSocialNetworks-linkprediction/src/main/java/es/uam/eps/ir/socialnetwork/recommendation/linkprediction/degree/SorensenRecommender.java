 ...
 */
public class SorensenRecommender<U> extends UserFastRankingRecommender<U> 
{
    /**
     ...
     */
    public SorensenRecommender(FastGraph<U> graph, EdgeOrientation uSel, EdgeOrientation vSel) 
    {
        super(graph);
        this.uSel = uSel;
        this.vSel = vSel;
    ...
    public Int2DoubleMap getScoresMap(int uidx) 
    {
        ...
        this.uIndex.getAllUsers().forEach(v -> {
            long intersect = 2*this.getGraph().getNeighbourhood(v, vSel).filter(uNeigh::contains).count();
