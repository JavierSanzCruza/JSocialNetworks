 ...
 */
package es.uam.eps.ir.socialnetwork.recommendation.reranking.global.swap;
...
public abstract class SwapGreedyReranker<U,I> extends SwapReranker<U,I> 
{
    ...
    protected int[] rerankPermutation(Recommendation<U, I> rec, int maxLength)
    {
        ...
        IntStream.range(permSize, list.size()).forEach(i -> remainingI.add(i));
        IntStream.range(0, permSize).forEach(i -> perm.add(i));
    ...
    private Recommendation<U, I> permuteRecommendation(Recommendation<U, I> rec, int[] perm) 
    {
        ...
        {
            Tuple2od<I> t = new Tuple2od<>(from.get(perm[i]).v1, (double) (perm.length - i));
            to.add(t);
        }
