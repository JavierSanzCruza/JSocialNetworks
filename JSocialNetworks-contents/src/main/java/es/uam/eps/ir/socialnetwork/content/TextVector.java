...
public class TextVector 
{
    ...
    public TextVector(Stream<TextVector> textVectors, int num)
    {
        ...
        this.terms.defaultReturnValue(0.0);
        textVectors.forEach(vector ->
        {
            vector.terms.entrySet().forEach(entry -> {
                String term = entry.getKey();
                double weight = entry.getValue() / (num + 0.0);
    ...
    {
        return this.terms.entrySet().stream().map(entry -> new Tuple2oo<>(entry.getKey(), entry.getValue()));
    }
    ...
    public double scalarProduct(TextVector vector)
    {
        ...
        return smaller.terms.entrySet().stream().mapToDouble(entry -> 
        {
            String term = entry.getKey();
    ...
    {
        Object2DoubleMap<String> map = new Object2DoubleOpenHashMap<>();
        for(Entry<String, Double> entry : this.terms.entrySet())
        {
