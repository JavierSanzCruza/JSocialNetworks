...
public class Closeness<U> implements VertexMetric<U>
{
    ...
    public double compute(Graph<U> graph, U user) {
        ...
        {
            case HARMONICMEAN:
                value = this.dc.getDistances(user).values().stream().mapToDouble(dist -> (dist.isInfinite() || dist == 0) ? 0.0 : 1.0/dist).sum();
    ...
    public double averageValue(Graph<U> graph) 
    {
        ...
        return map.values().stream().mapToDouble(val -> val).average().getAsDouble();
    }
}
